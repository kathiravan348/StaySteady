...
`packages/ui` (`@staysteady/ui`) is completely decoupled from domain DTOs and application code. It exposes 41 headless, accessible components with SCSS modules referencing CSS custom properties.
...
3. **Workbench Features & Controls**:
   ...
   - **Search & Filter**: Search through the 38 interactive component stories across 7 categories:
     - **Primitives**: Button, Input, Select, Checkbox, Toggle, Badge, Icon, Spinner, Tooltip, Skeleton.
     - **Composites**: FormField, DropdownMenu, Modal, Drawer, Tabs, Accordion, Toast, Popover, CommandPalette.
...
A verification script verifies that `packages/ui`:

- Has **zero imports** from `apps/web` or domain schemas.
- Meets file length constraints ($\le 300$ lines).
- Correctly exports all 41 components and hooks from `packages/ui/src/index.ts`.

Run the test suite:

```bash
npx tsx C:\Users\kathiravan\.gemini\antigravity-ide\brain\7a575645-aee6-4e9a-b2bc-28d70f62a31c\scratch\verify_stage_l.ts
```
